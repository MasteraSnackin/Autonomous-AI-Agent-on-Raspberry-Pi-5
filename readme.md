...
This project turns a Raspberry Pi 5 (8 GB) into a production-grade autonomous AI agent using **OpenClaw** with Google Gemini 2.5 Flash and optional local models via Ollama. It supports both microSD-only and NVMe-accelerated setups; NVMe is optional but recommended for 24/7 or local-model use.
...
Set boot order:
```bash
sudo raspi-config # 6 Advanced Options -> A4 Boot Order -> B2 NVMe/USB Boot
```

Shut down, physically remove the microSD card, and power the Pi back on.

Enable PCIe Gen 3 for maximum throughput:
```bash
sudo nano /boot/firmware/config.txt
```
...
On NVMe, 8 GB swap is safe and fast for this workload. On microSD it will be slower and will wear the card more quickly, so consider reducing swap size or moving to NVMe for long-term or 24/7 use.
...
You should now see roughly 8 GiB of swap available. This gives the OpenClaw gateway and Node processes headroom when memory usage spikes.

---

## 5. Optional: local models with Ollama

This is optional but lets you run fully local models when you do not want to use cloud APIs.
...
On a Pi 5, 3B-class models are a practical upper bound for general use; larger models will run noticeably slower and demand more thermal headroom. You can later point OpenClaw at this local Ollama endpoint for a hybrid cloud/local setup.
...
Install OpenClaw globally, relaxing specific C compiler warnings just for this command so native audio modules build cleanly on ARM64:
```bash
...
```
This script sets up OpenClaw, dependencies and systemd services for you. The rest of this guide shows the manual, more controllable path.

### 6.3 Manual agent and system configuration

You will bypass the interactive onboarding initially and seed a known-good Gemini + Telegram config. You will plug in your own API keys and IDs; do not commit these files to git.
...
EOF
```
Note: Google are rolling out newer Gemini embedding models; `text-embedding-004` remains valid in many setups but check current model IDs and deprecation timelines and adjust accordingly.
...
EOF
```
The default gateway bind is loopback-only, which is what you want for a safe local deployment.

### 6.4 Run OpenClaw onboarding (daemon mode)

Now run the onboarding wizard and install OpenClaw as a system daemon:
```bash
...
After onboarding completes, you should see the gateway connected in the TUI. At this point your Pi agent is running as a background service.
...
```
The gateway should be listening on `localhost:18789` as configured.
...
In Telegram, start a chat with **@BotFather**. Use `/newbot` to create a bot and follow the prompts.

Copy the bot token and add it in your OpenClaw Telegram channel configuration (via TUI/web UI or config files). Find your Telegram user ID (for example using a “user ID” bot) and ensure it is listed under `admins` and `allowedUsers` in your agent config.

**Never commit your bot token, user IDs, or API keys to git.**

### 7.2 Pair your account

Open your bot in the Telegram app and click **Start**. The bot will respond with a pairing code such as `A1B2-C3D4`.
...
Return to Telegram and say “Hi” to your bot. You should now have a fully autonomous Gemini-powered agent running on your Raspberry Pi.
...
| **Ollama 3B model** | On the Pi (NVMe) | Private, can run offline, no API fees | Slower, heavier CPU and RAM usage |

For light, cloud-only use (Gemini only, no heavy local models), a good microSD card is acceptable and keeps hardware cost low. For local models, persistent memory, or heavy logging, an NVMe SSD is the preferred storage backend on the Pi 5 and makes the system feel much more responsive.

OpenClaw’s security guidance recommends using a modern, strong model for any agent that can run tools or touch files and networks; Gemini 2.5 Flash fits that role well. If you use smaller or local models, limit tool access and sandbox the filesystem accordingly.

By default this guide configures Gemini for best responsiveness, while the NVMe + swap + cooling optimisations ensure the Pi can also handle modest local models if you choose to configure OpenClaw with Ollama later.
...
- **OpenClaw install fails on @discordjs/opus or audio libs:** Re-run the `npm install` with the `CFLAGS`/`CXXFLAGS` snippet and check that `libopus-dev` is installed.
- **Gateway service not running:** Run `openclaw status` and inspect logs with `journalctl -u openclaw-gateway`.
- **Local model is very slow or times out:** Try a smaller Ollama model, reduce concurrent sessions, or switch heavy tool-use flows back to Gemini.
